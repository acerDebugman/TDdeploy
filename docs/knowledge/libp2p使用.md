...
Behavior 是可以组合的，内部通过 poll 方法的各种 feature 叠加；
...
**Connection Established 一定在 Identify Received 之前**。

这是 libp2p 协议栈的分层架构决定的：

```
1. 传输层握手（TCP/QUIC）
   ↓
2. 安全层握手（Noise/TLS）→ 验证 Peer ID
   ↓
3. 多路复用协商（Yamux/Mplex）
   ↓
🔥 Connection Established（连接已建立）
   ↓
4. 应用层协议启动 → Identify 协议自动运行
   ↓
5. 发送/接收 identify 消息
   ↓
🔥 Identify Received（收到身份信息）
```
